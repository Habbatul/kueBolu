...
<div class="isi">
     <table class="tabel">
                  <h2 style="color:gray;padding-top: 10px;">Laporan Pendapatan Tiap Bulan</h2>
                             
                                        <tr class="tabel">
                                            <th class="tabel">Bulan & Tahun</th>
                                            <th class="tabel">Jumlah Penghasilan</th>
                                        </tr>
                                
                                        <?php
                                        $transaksi = mysqli_query($conn, "select concat (MONTH(tanggal),'/',YEAR(tanggal)) as TahunDanBulan, sum(total_harga) as JumlahHasilBulanan FROM lap_penjualan GROUP BY YEAR(tanggal),MONTH(tanggal)");

                                        while ($data = mysqli_fetch_array($transaksi)) {
                                            $id_bulan = $data['TahunDanBulan'];
                                            $id_HasilBulanan = $data['JumlahHasilBulanan'];
                                        ?>
                                            <tr>
                                                <td class="tabel"><?= $id_bulan ?></td>
                                                <td class="tabel">Rp <?= $id_HasilBulanan ?></td>
                                            </tr>
                                          <?php
                                          }
                                          ?> 
